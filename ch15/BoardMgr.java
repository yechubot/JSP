...
public class BoardMgr {
	...
	private static final String  SAVEFOLDER = "E:/jsp/myApp/WebContent/ch15/fileupload";
	...
	//총 게시물수
	public int getTotalCount(String keyField, String keyWord) {//totalCount 리턴
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		...
		try {
			...
			e.printStackTrace();//exception 메시지 출력 
		} finally {
			pool.freeConnection(con, pstmt, rs);
		}
		return totalCount;
	...
	// 게시물 리턴
	public BoardBean getBoard(int num) {//번호 넘어옴
		Connection con = null;//미리 만든 커넥션 사용
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		BoardBean bean = new BoardBean();
		try {
			...
			rs = pstmt.executeQuery();
			if (rs.next()) {
				//빈에 기록함(저장)-- 여러군데에서 사용할 거라서 다 가져옴
				bean.setNum(rs.getInt("num"));
				bean.setName(rs.getString("name"));
				bean.setSubject(rs.getString("subject"));
	...
	public void upCount(int num) {
		...
		try {
			...
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);//해당 번호 조회수 1증가해
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	// 게시물 답변
	public void replyBoard(BoardBean bean) {//reBean을 bean으로 받음
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		try {
			...
			//값 추가
			sql += "values(?,?,?,?,?,?,now(),?,0,?)";
			int depth = bean.getDepth() + 1;//2칸씩 밀려서 추가됨
	...
	public void replyUpBoard(int ref, int pos) {
		//ref ->답변 출처글 번호 : 어느글의 답변이다를 알수 있게
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		try {
